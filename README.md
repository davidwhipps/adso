...
- **Local web UI** — `pip install ".[web]"`, then `adso serve` opens a browser view over the same catalogue: browse the library by shelf, smart view (loaned out, unrated, recently added…) or tag as a cover grid, a table or a one-screen wall; open a book in a sidebar to edit its tags, format, loan and notes; select several books to tag them at once. Also visual conflict resolution, import, activity, reports, and CSV/JSON/Notion export. Press `?` in the library for keyboard shortcuts.

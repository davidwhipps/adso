...
- **Novels stay novels.** Open Library files plenty of fiction under "History" or "Biography"; a nonfiction genre that only a subject puts on a book you've classed as fiction is simply left off (add it by hand if you want it). Over-broad matches can be removed with `adso taxonomy unalias` (or the × next to an alias on the Categories page).
...
- **Primary genres are picked for you**, never asked about: the one most-specific genre; else the genre from your own shelf over one from Open Library; else the parent that competing subgenres share (Cyberpunk + Space Opera → Science Fiction); else Adso picks, keeping its earlier pick, then preferring your shelves, the book's side of fiction/nonfiction, and the more specific, rarer genre. Change any book's lead genre on its page or with `adso edit --genre`, and your choice is final.
